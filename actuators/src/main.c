...
// void play_tone_for_seconds(const struct device *dev_i2s, int seconds)
// {
// 	int ret;
// 	void *tx_buf;

// 	// Calculate total samples to send: rate * seconds
// 	uint32_t total_samples = 96000 * seconds;
// 	uint32_t samples_per_block = BLOCK_SIZE / (sizeof(int16_t) * 2);  // Stereo: 2 channels
// 	uint32_t total_blocks = (total_samples + samples_per_block - 1) / samples_per_block;
...
// 		ret = k_mem_slab_alloc(&tx_0_mem_slab, &tx_buf, K_FOREVER);
// 		if (ret < 0) {
// 			printf("TX slab alloc failed\n");
// 			break;
// 		}

// 		fill_buf((int16_t *)tx_buf, i % 3);  // Optional modulation
// 		ret = i2s_write(dev_i2s, tx_buf, BLOCK_SIZE);
// 		if (ret < 0) {
// 			printf("TX write failed\n");
// 			k_mem_slab_free(&tx_0_mem_slab, &tx_buf);
// 			break;
// 		}
// 	}
...
// 	play_tone_for_seconds(dev_i2s, 10);
// 	printf("Done.\n");
